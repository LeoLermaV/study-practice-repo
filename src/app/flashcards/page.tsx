            ...
            {(['system-design', 'ddia', 'dsa', 'cs-fundamentals', 'behavioral'] as Category[]).map((c) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center justify-between rounded-xl bg-[#141414] border border-[#262626] px-5 py-4 text-left hover:bg-[#1c1c1c] transition-colors duration-200"
                >
                  <span className="font-medium">{catNames[c]}</span>
                  <span className="text-xs text-[#666]">{count} topics</span>
                </button>
              )
            })}
          </div>

          <p className="text-xs text-[#666] max-w-sm text-center leading-relaxed">
            Each topic becomes a flashcard. Rate your recall to schedule the next review.
            Use keyboard: <kbd className="px-1 py-0.5 rounded bg-[#1c1c1c] text-white text-[10px]">Space</kbd> to flip,{' '}
            <kbd className="px-1 py-0.5 rounded bg-[#1c1c1c] text-white text-[10px]">1-4</kbd> to rate.
          </p>
        </div>
      ) : (
        <div>
